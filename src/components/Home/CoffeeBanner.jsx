...
const CoffeeBanner = () => {
  ...
  const getProductImageUrl = (product) => {
      ...
      if (Array.isArray(arr) && arr.length > 0) {
        ...
        if (typeof first === "object" && first !== null) {
          rawUrl =
            ...
            first.url ||
            first.image ||
            (first.photo && (first.photo.url || first.photo.photo_url));
    ...
    if (rawUrl && typeof rawUrl === "object") {
      rawUrl = rawUrl.url || rawUrl.photo || rawUrl.photo_url;
    }
    ...
    const loadSpecialProduct = async () => {
      ...
      try {
        ...
        if (foundProduct) {
          ...
          const productId =
            foundProduct.id || foundProduct.product_id || foundProduct._id;
  ...
  const handleAddToCart = () => {
    ...
    const originalPrice =
      Number(selectedSupply.price) || Number(product.price) || 0;
    ...
    const updatedSupplies = product.supplies.map((supply, index) =>
      index === 0 ? { ...supply, price: discountedPrice } : supply,
    );
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    {formatPrice(
                      (specialProduct.supplies?.[0]
                        ? getPrice(specialProduct.supplies[0], currency)
                        : getProductPrice(specialProduct, currency)) * 0.85,
                      currency,
                    )}
                  </Typography>
                  ...
                  >
                    {formatPrice(
                      specialProduct.supplies?.[0]
                        ? getPrice(specialProduct.supplies[0], currency)
                        : getProductPrice(specialProduct, currency),
                      currency,
                    )}
                  </Typography>
                </Box>

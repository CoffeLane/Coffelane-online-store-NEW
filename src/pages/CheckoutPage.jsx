...
import {
  ...
  formatCardNumber,
  formatExpiry,
} from "../components/utils/formatters.jsx";
...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    if (user && !isAdmin) {
      ...
      setPhone((prev) =>
        !prev && user.phone_number ? formatPhone(user.phone_number) : prev,
      );
      ...
      setStreet((prev) =>
        !prev && user.street_name ? user.street_name : prev,
      );
      setZip((prev) => (!prev && user.zip_code ? user.zip_code : prev));
      setApartment((prev) =>
        !prev && user.apartment_number ? user.apartment_number : prev,
      );
    }
  }, [user, isAdmin]);
            ...
            if (discountCode?.code && orderId) {
              try {
                await apiWithAuth.get(
                  `/discount-codes/${discountCode.code}/${orderId}/`,
                );
              } catch (e) {
  ...
  const handleCompletePayment = async () => {
    ...
    if (!cardName.trim()) newErrors.cardName = "Card holder name required";
      ...
      newErrors.cardName =
        "Invalid card name. Please enter first and last name.";
    ...
    try {
      ...
      else if (backendError?.discount_code)
        errorMessage = `Promo code: ${backendError.discount_code[0]}`;
  ...
  const handleQuantityChange = (key, change, cartItem) => {
    ...
    if (change === 1)
      dispatch(addToCart({ product, quantity: 1, selectedSupplyId: supplyId }));
        ...
        >
          ...
          >
            ...
            {discountCode && (
              ...
              >
                <Typography
                  sx={{ color: "#09d05fff", fontSize: "14px", fontWeight: 600 }}
                >
            ...
            >
              ...
              <Typography sx={{ ...h5, fontSize: { xs: "14px", md: "16px" } }}>
                {formatPrice(total - discountAmount, currency)}
              </Typography>
            </Box>

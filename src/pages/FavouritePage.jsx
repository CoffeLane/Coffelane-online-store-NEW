...
// 👉 ВАЖНО: импортируй свою МОДАЛКУ ЛОГИНА
...
export default function FavouritePage() {
  ...
  // ⛔ Если НЕТ токена → показываем модалку
  useEffect(() => {
    if (!token) {
      setLoginOpen(true);
      return;
  ...
  // 🛑 Пока юзер не авторизован — возвращаем только модалку
  if (!token) {
    return <LoginModal open={loginOpen} handleClose={() => setLoginOpen(false)} />;
  }

  // --- Дальше идёт тот же код страницы избранного ---
  const allFavorites = useMemo(() => {
    if (favorites.length === 0) return [];
    const shuffled = [...favorites].sort((a, b) => {
      const keyA = `${a.type || "unknown"}-${a.id}`;
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: "My Favorite Products", text: shareText, url: shareUrl });
      } catch {
        copyToClipboard(shareUrl);
      }
    } else {
              ...
              return (
                ...
                >
                  ...
                  <CardContent sx={{ flex: 1, display: "flex", flexDirection: "column" }}>
                    ...
                    <Typography
                      sx={{ mt: 1, color: "#16675C", fontSize: 14, fontWeight: 700, textAlign: "right", mb: 1 }}
                    >
                      ${price.toFixed(2)}
                    </Typography>
                    ...
                    <Button
                      ...
                      sx={isInCart ? btnInCart : btnCart}
                      endIcon={
                        <Box component="img" src={isInCart ? incart : shopping} alt="" sx={{ width: 24, height: 24 }} />
                      }
                    >
                      {isInCart ? "In cart" : "Add to bag"}
                    </Button>

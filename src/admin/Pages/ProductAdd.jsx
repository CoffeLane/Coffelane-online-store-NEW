  ...
  const handleSaveProduct = async () => {
    ...
    try {
      ...
      if (productType === "product") {
        ...
        const weightNum =
          weight && weight.trim().length > 0 ? parseFloat(weight.trim()) : 0;
        const stockNum =
        ...
        productData.visible = visible !== false; 
        ...
        console.log(
          "⏳ Waiting 1 second for server to process product creation...",
        );
        await new Promise((resolve) => setTimeout(resolve, 1000));

        if (
          (images.length > 0 && images.some((img) => img.file)) ||
          cover?.file
        ) {
          ...
          imagesToSend.forEach((img) => {
            if (img.file) {
              ...
              if (isValidFormat) {
                ...
                console.log(
                  `✅ Added image to FormData: ${img.file.name} (${fileType}, ${img.file.size} bytes)`,
                );
              } else {
                console.warn(
                  `Unsupported file format: ${fileType}. Only JPEG/JFIF/PNG are supported. File: ${img.file.name}`,
                );
              }
            }
          });
          ...
          if (
            cover?.file &&
            !imagesToSend.some((img) => img.file === cover.file)
          ) {
            ...
            if (isValidFormat) {
              ...
              console.log(
                `✅ Added cover to FormData: ${cover.file.name} (${fileType}, ${cover.file.size} bytes)`,
              );
            } else {
              console.warn(
                `Unsupported cover file format: ${fileType}. Only JPEG/JFIF/PNG are supported. File: ${cover.file.name}`,
              );
            }
          }
          ...
          console.log(
            "📤 Sending empty photo_ids and image_ids arrays for new product",
          );
          ...
          for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
              if (attempt > 0) {
                console.log(
                  `🔄 Retrying photo upload via PATCH (attempt ${attempt + 1}/${maxRetries + 1})...`,
                );
              ...
              photoResponse = await apiWithAuth.patch(
              ...
              console.log(
                "✅ Photos uploaded successfully via PATCH /products/product/{id}",
              );
              ...
              if (isRetryableError && attempt < maxRetries) {
                console.warn(
                  `PATCH request failed with ${patchError.response?.status || patchError.code || patchError.message}, retrying...`,
                );
                continue;
              } else {
          ...
          if (!photoResponse) {
            throw (
              lastError ||
              new Error("Failed to upload photos after all retries")
            );
          }
          ...
          const hasPhotosInResponse =
            (photoResponse.data?.photos_url &&
              photoResponse.data.photos_url.length > 0) ||
            (photoResponse.data?.product_photos &&
              photoResponse.data.product_photos.length > 0);

          if (!hasPhotosInResponse) {
            console.warn(
              "Photos uploaded but not found in response. Photos may be processed asynchronously on the server.",
            );
            console.warn(
              "This might indicate that PATCH doesn't save photos for new products. Trying alternative approach...",
            );
            ...
            try {
              ...
              if (cover?.file) {
                ...
                if (
                  isValidFormat &&
                  !imagesToRetry.some((img) => img.file === cover.file)
                ) {
              ...
              if (Array.from(retryPhotoFormData.entries()).length > 0) {
                const retryResponse = await apiWithAuth.patch(
                  `/products/product/${productId}`,
                  retryPhotoFormData,
                  {
                    headers: {
            ...
            for (let waitAttempt = 0; waitAttempt < 3; waitAttempt++) {
              ...
              console.log(
                `⏳ Waiting ${waitTime}ms for server to process photos (attempt ${waitAttempt + 1}/3)...`,
              );
              ...
              try {
                ...
                const hasPhotos =
                  updatedProduct.data?.photos_url?.length > 0 ||
                  updatedProduct.data?.product_photos?.length > 0;
                console.log(
                  `📸 Updated product data from /products/{id} (attempt ${waitAttempt + 1}):`,
                  {
                ...
                console.warn(
                  `⚠️ Could not fetch updated product data (attempt ${waitAttempt + 1}):`,
      ...
      if (err.response?.data) {
        ...
        if (err.response.data.flavor_profiles) {
          console.error(
            "Flavor profiles error:",
            err.response.data.flavor_profiles,
          );
        }
      }
      ...
      const errorMessage =
        ...
        (err.response?.data?.sku
          ? `SKU: ${Array.isArray(err.response.data.sku) ? err.response.data.sku[0] : err.response.data.sku}`
          : "") ||
        (err.response?.data?.supplies
          ? `Supplies: ${Array.isArray(err.response.data.supplies) ? err.response.data.supplies[0] : err.response.data.supplies}`
          : "") ||
        (err.response?.data?.flavor_profiles
          ? `Flavor profiles: ${Array.isArray(err.response.data.flavor_profiles) ? err.response.data.flavor_profiles[0] : err.response.data.flavor_profiles}`
          : "") ||
